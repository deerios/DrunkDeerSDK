...
/// Standard precision (B6 × 10):   1 unit = 0.1 mm    -> multiply mm × 10
/// Kun precision    (B6 × 100):    1 unit = 0.01 mm   -> multiply mm × 100
/// FD precision     (FD × 200):    1 unit = 0.005 mm  -> multiply mm × 200
/// </summary>
[TestFixture]
public class DepthConversionTests
{
	...
	[TestCase(1.0f, (byte)100)]
	[TestCase(0.1f, (byte)10)]
	[TestCase(2.0f, (byte)200)]
	[TestCase(0.01f, (byte)1)]
	[TestCase(0.15f, (byte)15)]
	[TestCase(0.005f, (byte)1)] // rounds: 0.005 × 100 = 0.5 -> 1 (round half to even -> 0 in banker's, but Math.Round rounds away for .5)
	public void MmToKunUnit_ConvertsCorrectly(float mm, byte expected)
	{
		Assert.That(KeyboardSession.MmToKunUnit(mm), Is.EqualTo(expected));
	}

	[Test]
	public void MmToKunUnit_NegativeInput_ClampsToZero()
	{
		Assert.That(KeyboardSession.MmToKunUnit(-1.0f), Is.EqualTo(0));
	}

	[Test]
	public void MmToKunUnit_VeryLargeInput_ClampsTo255()
	{
		Assert.That(KeyboardSession.MmToKunUnit(100f), Is.EqualTo(255));
	}
